...
/* portion of code to be split by threads */
void* threadedFunction(void* num)
{
  int* threadID = (int*)num;
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    for( int px = *threadID; px < width; px += nthreads  )
      {
...
main( int argc, char **argv )
{
  ...
  int *threadNum = (int*) malloc(nthreads*sizeof(int));
  //memset(threadNum, 0, nthreads*sizeof(int));
  
  int i;
  /* using memset triggered warnings on compilation, therefore I use
     a loop to initialize the array to zero */
  for(i = 0; i < nthreads; i++)
    *(threadNum + i) = 0;   

  for(i = 0; i < nthreads; i++) {
    pthread_create(threads + i, NULL, threadedFunction, (threadNum + i));
    *(threadNum + i + 1) = *(threadNum + i) + 1;
  }

  for(i = 0; i < nthreads; i++)
    pthread_join(threads[i], NULL);  //wait and join threads when done
